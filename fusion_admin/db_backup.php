...
if(isset($_POST['btn_create_backup'])){
	...
	if(count($db_tables)>0){
		...
		foreach($db_tables as $table){
			...
			$row=dbarray(dbquery("SHOW CREATE TABLE $table"));
			echo $row[1].";".$crlf;
	...
	opentable(LAN_410);
	echo "<form action=\"$PHP_SELF\" name=\"frm_info\" method=\"post\">
<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" align=\"center\" class=\"body\">
<tr>
...
<td>".parsesize(get_database_size())." (".get_table_count()." tables)</td>
</tr>
<tr>
<td class=\"alt\" align=\"right\">".LAN_455."</td>
<td>".parsesize(get_database_size($fusion_prefix))." (".get_table_count($fusion_prefix)." tables)</td>
</tr>
<tr>
	...
	if($dh!=false){
		...
		if(!empty($filelist)){
			...
			foreach($filelist as $key=>$file){
				...
				echo "<td align=\"right\">".parsesize(@filesize("$dbbackupdir$file[1]"))."</td>";
				echo "<td align=\"right\">";
