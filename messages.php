...
<table border='0' cellpadding='0' cellspacing='0' width='100%'>
<tr>
<td class='tbl'><input type='button' value='".$locale['435']."' class='button' onclick=\"location.href='".FUSION_SELF."?folder=".$folder."'\"></td>
<td align='right' class='tbl'>
...
<input type='submit' name='send_message' value='".$locale['430']."' class='button'>
		...
		opentable($locale['480']);
		echo "<div align='center'><br>".$locale['482']."<br><br></div>\n";
	}
	...
	if (isset($_POST['chk_sendtoall'])) {
		...
		if ($limit_inbox == "0" || ($cnt_inbox < $limit_inbox)) {
			$result=dbquery("INSERT INTO ".$db_prefix."messages (message_to, message_from, message_subject, message_message, message_smileys, message_read, message_datestamp, message_folder) VALUES('".$msg_to."','".$sender_id."','".$subject."','".$message."','".$smileys."','0','".time()."','0')");
		} else {
			$error = $locale['628'];
		}
		// Get user options
...
<table cellpadding='0' cellspacing='1' width='100%' class='tbl-border'>
<tr>
<td align='right' class='tbl2' width='1%' style='white-space:nowrap'>".$locale['421'].":</td>
<td class='tbl1'>".$recipient."</td>
</tr>
<tr>
<td align='right' class='tbl2' style='white-space:nowrap'>".$locale['405'].":</td>
<td class='tbl1'><input type='text' name='subject' value=\"".stripslashes($subject)."\" maxlength='32' class='textbox' style='width:400px;'></td>
</tr>
<tr>
<td align='right' class='tbl2' valign='top' style='white-space:nowrap'>".$locale['422'].":</td>
<td class='tbl1'>".$orig_message."<br>
<span class='small alt'>".sprintf($locale['440'], $msgdate)."</span></td>
</tr>
<tr>
<td align='right' class='tbl2' valign='top' style='white-space:nowrap'>".(0?$locale['422']:$locale['433']).":</td>
<td class='tbl1'><textarea id='message' name='message' cols='80' rows='15' class='textbox'>".stripslashes($message)."</textarea></td>
</tr>
<tr>
<td align='right' class='tbl2' valign='top'></td>
<td class='tbl1'>
...
<input type='submit' name='reply_message' value='".$locale['437']."' class='button'>
	...
	if (isset($msg_to)&&(((int)$msg_to) == $msg_to)){
		...
		// Check if user has reached inbox limit
		if ($limit_inbox == "0" || ($cnt_inbox < $limit_inbox)) {
			$result = dbquery("INSERT INTO ".$db_prefix."messages (message_to, message_from, message_subject, message_message, message_smileys, message_read, message_datestamp, message_folder) VALUES ('".$msg_to."', '".$userdata['user_id']."', '".$subject."', '".$message."', '".$smileys."', '0', '".time()."', 0)");
		} else {
			$error = $locale['628'];
		}
		// Get user options
	...
	if (isset($_POST['srch_reset'])) {
		...
		if ($srch_fields[$srch_type][0]) { $srch_text2 = stripinput($srch_text); } else { $srch_text2 = stripinput($srch_text); }
	...
	if ($srch_text != ""){
		$title = sprintf($locale['458'], stripinput($srch_text), $srch_fields[$srch_type][2]);
	} elseif (isset($show)) {
		($show != "all" && preg_match("/^[a-z]$/", $show) ? $title = sprintf($locale['459'], $show) : "");
	} else {
		$title = "";
	}
	if(isset($sort_type)){
...
<tr>
<td colspan='3' class='tbl'><span class='small2'>".$title."</span></td>
</tr>
</table><br>\n";
	}
	opentable($locale['400']);
	...
	if ($cnt_total !=0 || $folder != "settings"){
		tablebreak();
		echo $title."<table cellpadding='0' cellspacing='0' width='100%'>
<tr>
<td class='tbl'><input type='button' class='button' title='".$locale['500']."' onclick=\"location.href='".FUSION_SELF."?folder=".$folder."&amp;msg_send'\" value='".$locale['401']."'></td>
<td width='1%' class='tbl'></td>
<td width='1%' align='right' class='tbl'>
<table border='0' cellpadding='0' cellspacing='1' width='100%' class='tbl-border'>
<tr>
<td width='25%' class='".($folder=='inbox'?'tbl2':'tbl1')."' align='center' style='white-space:nowrap'>\n".$folder_stat[0]."<a href='".FUSION_SELF."?folder=inbox' title='".$locale['501']."'>".$locale['402'].$inbox_display."</a>".$folder_stat[1]."\n</td>\n
<td width='25%' class='".($folder=='sentbox'?'tbl2':'tbl1')."' align='center' style='white-space:nowrap'>\n".$folder_stat[2]."<a href='".FUSION_SELF."?folder=sentbox' title='".$locale['502']."'>".$locale['403'].$sentbox_display."</a>".$folder_stat[3]."\n</td>\n
<td width='25%' class='".($folder=='savebox'?'tbl2':'tbl1')."' align='center' style='white-space:nowrap'>\n".$folder_stat[4]."<a href='".FUSION_SELF."?folder=savebox' title='".$locale['503']."'>".$locale['404'].$savebox_display."</a>".$folder_stat[5]."\n</td>\n
<td width='25%' class='".($folder=='options'?'tbl2':'tbl1')."' align='center' style='white-space:nowrap'>\n".$folder_stat[6]."<a href='".FUSION_SELF."?folder=options' title='".$locale['516']."'>".$locale['620']."</a>".$folder_stat[7]."\n</td>\n
</tr>
</table>
</td>
</tr>
...
<table cellpadding='0' cellspacing='1' width='500px' class='tbl-border'>
<tr>
...
<td class='tbl2' width='1%' align='center'>".$locale['407']."</td>
				...
				while ($data = dbarray($result)) {
					...
					echo "</td>\n<td class='tbl1' align='center' style='white-space:nowrap'>".$msgdate."</td>\n";
					$r++;
					echo "</tr>\n";
				}
				echo "</table>
<script type='text/javascript'>
	...
	function ValidateForm(dml,chkName){
		...
		return false;
				...
				echo "<div align='center'><br>".($srch_where==""?$locale['461']:$locale['453'])."<br><br></div>\n";
			}
			if($msgcount>0){
				tablebreak();
				echo "<table cellpadding='0' cellspacing='0' width='100%'><tr>\n
<td class='tbl'><span class='small'><input type='button' class='button' title='".$locale['507']."' onclick=\"javascript:setChecked('frm_privmsg','chk_mark[]',1);\" value='".$locale['410']."'>
<input type='button' class='button' title='".$locale['508']."' onclick=\"javascript:setChecked('frm_privmsg','chk_mark[]',0);\" value='".$locale['411']."'></span></td>\n
<td align='right' class='tbl'>".$locale['409']." \n";
	if($folder=='savebox'){
...
<tr><td colspan='20' align='center' class='tbl'>\n".$locale['417']." <a href='".FUSION_SELF."?folder=".$folder."&amp;show=all' title='".$locale['509']."'>".$locale['418']."</a>";
	...
	if($msgcount>0 || $srch_where!=""){
				...
				if (isset($show)){
					...
					$link = "?srch_text=".stripinput($srch_text)."&amp;srch_type=".$srch_type."&amp;";
				} else {
					$link = "?";
				}
				...
				if ($rows != 0) echo "<div align='center' style='margin:5px;'>".makePageNav($rowstart,$itemsperpage,$rows,3,FUSION_SELF.$url)."</div>\n";
				opentable($locale['450']);
				echo "<form method='get' action='".FUSION_SELF."'>
<input type='hidden' name='folder' value='".$folder."'>
<table align='center' cellspacing='0' cellpadding='0' class='body'>
<tr><td valign='top' align='right' class='tbl'>".$locale['454']."&nbsp;</td>
<td class='tbl'><input type='text' name='srch_text' class='textbox' value=\"".$srch_text."\" style='width:150px'><br>
<span class='small'>".$locale['457']."</span><br><br></td></tr>
<tr><td valign='top' align='right' class='tbl'>".$locale['455']."&nbsp;</td><td>\n
<select name='srch_type' class='textbox' style='width:150px'>\n";
				...
				echo "</select></td></tr><tr><td valign='top' align='right' class='tbl'>".$locale['456']."&nbsp;</td><td>\n
				...
				echo "</select></td></tr><tr><td align='right' class='tbl'> </td><td class='tbl'><br>
		...
		echo "<div align='center'><br>".$locale['419']."<br><br>
<input type='button' class='button' title='".$locale['500']."' onclick=\"location.href='".FUSION_SELF."?folder=".$folder."&msg_send'\" value='".$locale['401']."'>
<br><br></div>\n";
	}
}
closetable();
