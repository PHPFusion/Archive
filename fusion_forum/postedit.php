...
if (isset($_POST['previewchanges'])) {
	...
	$udata = dbarray(dbquery("SELECT * FROM ".$fusion_prefix."users WHERE user_id='".$pdata['post_author']."'"));
	$is_mod = in_array($udata['user_id'], $forum_mods) && $udata['user_level'] < 251 ? true : false;
...
<span class='alt'>".($is_mod ? USERF1 : getuserlevel($udata['user_level']))."</span><br><br>
<span class='alt'>".LAN_423."</span> ".$udata['user_posts']."<br>
<span class='alt'>".LAN_424."</span> ".$udata['user_location']."<br>
<span class='alt'>".LAN_425."</span> ".showdate("%d.%m.%y", $udata['user_joined'])."</td>
<td class='tbl1'><span class='small'>".LAN_426.showdate("forumdate", $pdata['post_datestamp'])."</span></td>
</tr>
<tr>
...
<a href='index.php'>".LAN_449."</a><br><br>\n</center>\n";
		...
		echo "<a href='viewthread.php?forum_id=$forum_id&thread_id=$thread_id".$rstart."#".$post_id."'>".LAN_447."</a> |
...
<a href='index.php'>".LAN_449."</a><br><br>
	...
	opentable(LAN_408);
	echo "<form name='inputform' method='post' action='".FUSION_SELF."?action=edit&forum_id=$forum_id&thread_id=$thread_id&post_id=$post_id'>
