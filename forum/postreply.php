...
if (dbrows($result) != 0) {
	$data = dbarray($result);
	$caption = stripslashes($data[thread_subject]);
}
...
if (isset($_POST['previewreply'])) {
	...
	if ($subject == "") {
		$previewsubject = "RE: ".stripslashes($data[thread_subject]);
	} else {
...
<a href=\"index.php\">".LAN_248."</a><br><br>
	...
	if (isset($quote)) {
		...
		if (dbrows($result) != "0") {
			...
			$message = "[quote][b]$postee[1] wrote:[/b]\n".stripslashes($data[post_message])."[/quote]";
		}
	}
	opentable(LAN_203.": $caption");
...
<td class=\"forum1\"><input type=\"checkbox\" name=\"showsig\" value=\"sig\"$sigchecked> Show My Signature in this Post</td>
</tr>\n";
