...
echo ($data['user_avatar'] ? "<img src='".IMAGES."avatars/".$data['user_avatar']."' alt='".$locale['u017']."'>" : $locale['u046'])."\n</td>\n";
...
$result = dbquery("SELECT user_id,user_name FROM ".$db_prefix."users WHERE user_id<'$lookup' LIMIT 1");
if (dbrows($result)) { $data = dbarray($result); if ($data['user_id'] != $lookup) { $prev = "<a href='".BASEDIR."profile.php?lookup=".$data['user_id']."' title='".$data['user_name']."'><< Previous</a>"; }}
$result = dbquery("SELECT user_id,user_name FROM ".$db_prefix."users WHERE user_id>'$lookup' LIMIT 1");
if (dbrows($result)) { $data = dbarray($result); if ($data['user_id'] != $lookup) { $next =  "<a href='".BASEDIR."profile.php?lookup=".$data['user_id']."' title='".$data['user_name']."'>Next >></a>"; }}

if (isset($prev) || isset($next)) {
	echo "<table align='center' cellpadding='0' cellspacing='0' width='100%'>\n";
	if (isset($prev)) echo "<td class='tbl'>$prev</td>\n";
	if (isset($next)) echo "<td align='right' class='tbl'>$next</td>\n";
	echo "</tr>\n</table>\n";
}
