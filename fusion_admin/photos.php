...
require fusion_langdir."admin/admin_photos.php";
require "navigation.php";

function builduseroptionlist($selected_user_id=1){
	global $fusion_prefix;
	$user_option_list="";
	$levels = array_reverse(array(1=>USER1, USER2, USER3, USER4), true);
	$modlevel = $levels[$modlevel];
	foreach($levels as $level=>$modlevel){
		$uresult=dbquery("SELECT * FROM ".$fusion_prefix."users WHERE user_mod='$level' ORDER BY user_name");
		if(dbrows($uresult)>0){
...
if(!Admin()){
	...
	header("Location: $PHP_SELF?album_id=$album_id");
}elseif(($action == "deletephotopic") && isset($photo_id)) {
	if(file_exists("$gallery_dir/$photo_id.jpg")) unlink("$gallery_dir/$photo_id.jpg");
	if(file_exists("$gallery_dir/$photo_id"."t.jpg")) unlink("$gallery_dir/$photo_id"."t.jpg");
	header("Location: $PHP_SELF?action=editphoto&photo_id=$photo_id");
}elseif($action=="deletephoto"){	// delete photo
	if(isset($photo_id)){
	...
	if(dbrows($result)!=0){
		...
		if(is_uploaded_file($_FILES['photo_pic_file']['tmp_name'])){
			if($_FILES['photo_pic_file']['size']>=$settings['album_max_b']){
				...
				if(file_exists("$gallery_dir/$photo_id.jpg")) unlink("$gallery_dir/$photo_id.jpg");
				if(file_exists("$gallery_dir/$photo_id"."t.jpg")) unlink("$gallery_dir/$photo_id"."t.jpg");
				move_uploaded_file($_FILES['photo_pic_file']['tmp_name'], "$gallery_dir/$photo_id.jpg");
				createThumbnail("$gallery_dir/$photo_id.jpg","$gallery_dir/$photo_id"."t.jpg",$settings['thumb_image_w'],$settings['thumb_image_h']);
			}
		}
	}
	if($error<>""){
		...
		echo"<a href='$PHP_SELF?album_id=$album_id'>".LAN_427."</a><br><br><a href='".fusion_basedir."index.php'>".LAN_428."</a><br><br></center>\n";
		closetable();
	}else{
		header("Location: $PHP_SELF?album_id=$album_id");
	}
	...
	echo "<form name='frm_add_photo' method='post' action='$PHP_SELF' enctype='multipart/form-data'>\n";
	...
	echo ")</span></td></tr>";
	if(file_exists("$gallery_dir/$photo_id"."t.jpg")){
		echo "<tr><td valign='top'>".LAN_406.":</td><td><img src='$gallery_url/$photo_id"."t.jpg' border='1' alt='".LAN_418."' width='".$settings['thumb_image_w']."' height='".$settings['thumb_image_h']."'></td></tr>\n";
	}
	echo "<tr><td valign='top'>".LAN_405.":";
	if(file_exists("$gallery_dir/$photo_id.jpg")){
		echo "<br><br><a class='small' href='$PHP_SELF?action=deletephotopic&photo_id=$photo_id'>".LAN_412."</a></td><td><img src='$gallery_url/$photo_id.jpg' border='1' alt='".LAN_418."'>";
	}else{
	...
	if($action=="editphoto" && $photo_comments>0){
		...
		if ($cnt_comments != 0) {
			...
			while ($data = dbarray($result)) {
				...
				echo strftime($settings['longdate'], $data['comment_datestamp']+($settings['timeoffset']*3600))."</span>\n";
				if ($i != $cnt_comments) {
					echo "<br><br>\n";
				} else {
					echo "\n";
...
<td align='right'>".LAN_411."</td>
			...
			while($data=dbarray($result)){
				...
				if($rows!=1){
					if($photo_order==1){
						$up_down=" <a href='$PHP_SELF?action=mdown&album_id=$album_id&photo_id=$photo_id'><img src='".fusion_themedir."images/down.gif' border='0' /></a>";
					}elseif($photo_order<$rows){
						$up_down=" <a href='$PHP_SELF?action=mup&album_id=$album_id&photo_id=$photo_id'><img src='".fusion_themedir."images/up.gif' border='0' /></a>\n";
						$up_down.=" <a href='$PHP_SELF?action=mdown&album_id=$album_id&photo_id=$photo_id'><img src='".fusion_themedir."images/down.gif' border='0' /></a>";
					}else{
						$up_down=" <a href='$PHP_SELF?action=mup&album_id=$album_id&photo_id=$photo_id'><img src='".fusion_themedir."images/up.gif' border='0' /></a>";
					}
...
<td><a href='$PHP_SELF?action=editphoto&photo_id=$photo_id'>$photo_title</a></td>
<td align='center'>";
				if(file_exists("$gallery_dir/$photo_id.jpg")){
				...
				echo "</td>\n
<td><a href='".fusion_basedir."profile.php?lookup=$photo_added_by'>$photo_added_by_name</a></td>
<td align='center'>".strftime($settings['shortdate'], $photo_date+($settings['timeoffset']*3600))."</td>
<td align='center'>$comment_count</td>
<td>$photo_order</td>
<td>$up_down</td>
<td align='right'><a href='$PHP_SELF?action=deletephoto&photo_id=".$data['photo_id']."'>".LAN_412."</a></td>";
				echo "</tr>";
				$r++;
			}
			echo "</table>\n";
