...
if ($stype == "l") {
	if (isset($_POST['submit_link'])) {
		if ($_POST['link_name'] != "" && $_POST['link_url'] != "" && $_POST['link_description'] != "") {
			...
			$result = dbquery("INSERT INTO ".$db_prefix."submissions VALUES('', 'l', '".$userdata['user_id']."', '".time()."', '".serialize($submit_info)."')");
...
<input type='submit' name='submit_link' value='".$locale['425']."' class='button'>
	...
	if (isset($_POST['submit_news'])) {
		if ($_POST['news_subject'] != "" && $_POST['news_body'] != "") {
			...
			$result = dbquery("INSERT INTO ".$db_prefix."submissions VALUES('', 'n', '".$userdata['user_id']."', '".time()."', '".addslashes(serialize($submit_info))."')");
			opentable($locale['400']);
			echo "<center><br>\n".$locale['460']."<br><br>
...
<input type='submit' name='submit_news' value='".$locale['475']."' class='button'></center>
	...
	if (isset($_POST['submit_article'])) {
		if ($_POST['article_subject'] != "" && $_POST['article_body'] != "") {
			...
			$result = dbquery("INSERT INTO ".$db_prefix."submissions VALUES('', 'a', '".$userdata['user_id']."', '".time()."', '".addslashes(serialize($submit_info))."')");
			opentable($locale['400']);
			echo "<center><br>\n".$locale['510']."<br><br>
...
<a href='index.php'>".$locale['412']."</a><br><br>\n</center>\n";
		...
		if (isset($_POST['preview_article'])) {
			...
			$article_description = descript(stripslash($_POST['article_description']));
...
<td class='tbl'><select name='article_category' class='textbox'>
$cat_list</select></td>
</tr>
<tr>
...
<td class='tbl'><textarea class='textbox' name='article_description' rows='3' style='width:300px;'>$article_description</textarea></td>
</tr>
<tr>
