...
if ($step == "add") {
	if (isset($_POST['add_user'])) {
		...
		if ($error == "") {
			$result = dbquery("INSERT INTO ".$fusion_prefix."users VALUES('', '$username', md5('$password1'), '$email', '$hide_email', '$user_location', '$user_birthdate', '$user_icq', '$user_msn', '$user_yahoo', '$user_web', '".$_POST['user_theme']."', '".$_POST['user_offset']."', '', '$user_signature', '0', '".time()."', '0', '1', '0')");
			opentable(LAN_480);
			echo "<center><br>
".LAN_481."<br><br>
...
<a href='index.php'>".LAN_433."</a><br><br>
		...
		opentable(LAN_480);
		echo "<form name='addform' method='post' action='$PHP_SELF?step=add' onSubmit='return ValidateForm(this)'>
...
<a href='index.php'>".LAN_433."</a><br><br>
		...
		while ($levels[$i]) {
			if (iUSER == "3" && iUSER != "4") {
				if ($i != "4") $modopts .= "<option value='$i'".($data[user_mod] == $i ? " selected" : "").">$levels[$i]</option>\n";
			} else if (iUSER == "4") {
				$modopts .= "<option value='$i'".($data[user_mod] == $i ? " selected" : "").">$levels[$i]</option>\n";
			}
			$i++;
		}
		if ($data['user_birthdate']!="0000-00-00") {
		...
		opentable(LAN_430);
		echo "<form name='inputform' method='post' action='$PHP_SELF?step=edit&user_id=$user_id' enctype='multipart/form-data'>
<table align='center' cellspacing='0' cellpadding='0' class='tbl'>
<tr>
...
<input type='text' value='".USER4."' class='textbox' onfocus='this.blur();' style='width:200px;'></td>\n";
}
} else {
echo "<td><select name='user_mod' class='textbox' style='width:200px;'>\n$modopts</select></td>\n";
}
...
<input type='submit' name='savechanges' value='".LAN_441."' class='button'></div>
</td>
</tr>
</table>
</form>\n";
...
<tr>
<td class='tbl2'>".LAN_401." [<a href='$PHP_SELF?step=add'>".LAN_402."</a>]</td>
...
<td align='center' style='text-indent:2px'>".getmodlevel($data['user_mod'])."</td>\n<td align='right'>";
			if (iUSER>=$data['user_mod'] && $data['user_id']!=1) {
				echo "<a href='$PHP_SELF?step=edit&user_id=".$data['user_id']."'>".LAN_406."</a>\n";
				if ($data['user_ban'] == "1") {
					echo "- <a href='$PHP_SELF?step=ban&act=off&sortby=$sortby&rowstart=$rowstart&user_id=".$data['user_id']."'>".LAN_407."</a>\n";
				} else {
					echo "- <a href='$PHP_SELF?step=ban&act=on&sortby=$sortby&rowstart=$rowstart&user_id=".$data['user_id']."'>".LAN_408."</a>\n";
				}
				echo "- <a href='$PHP_SELF?step=delete&sortby=$sortby&rowstart=$rowstart&user_id=".$data['user_id']."' onclick='return DeleteMember()'>".LAN_409."</a>";
			}
			echo "</td>\n</tr>\n";
		}
		echo "</table>\n";
	...
	echo "<td rowspan='2' class='tbl2'><a href='$PHP_SELF?sortby=all'>".LAN_411."</a></td>";
	for ($i=0;$search[$i]!="";$i++) {
		echo "<td align='center' class='tbl1'><div class='small'><a href='$PHP_SELF?sortby=".$search[$i]."'>".$search[$i]."</a></div></td>";
		echo ($i==17 ? "<td rowspan='2' class='tbl2'><a href='$PHP_SELF?sortby=all'>".LAN_411."</a></td>\n</tr>\n<tr>\n" : "\n");
	}
	echo "</tr>\n</table>\n";
	closetable();
	echo "<div align='center' style='margin-top:5px;'>\n".makePageNav($rowstart,20,$rows,3,"$PHP_SELF?sortby=$sortby&")."\n</div>\n";
