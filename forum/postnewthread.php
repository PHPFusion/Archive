...
if (isset($_POST['previewpost'])) {
	if (isset($_POST['sticky'])) { $sticky_check = " checked"; }
...
if (isset($_POST['postnewthread'])) {
	if (isset($_POST['sticky'])) { $sticky = "y"; } else { $sticky = "n"; }
	...
	if ($subject != "" && $message != "") {
		...
		$result = dbquery("INSERT INTO ".$fusion_prefix."threads VALUES('$forum_id', '', '$subject', '$userdata[user_id].$userdata[user_name]', '0', '0', '".time()."', '$userdata[user_id].$userdata[user_name]', '$sticky', 'n')");
...
<td class=\"forum1\">\n";
	if ($userdata[user_mod] > "0") {
		echo "<input type=\"checkbox\" name=\"sticky\" value=\"makesticky\" $sticky_check>".LAN_280."<br>\n";
	}
	echo "<input type=\"checkbox\" name=\"disable_smileys\" value=\"y\"$disable_smileys_check>".LAN_286;
