...
if (isset($_POST['previewchanges'])) {
	...
	if ($subject == "") {
		$previewsubject = "RE: ".stripslashes($data[post_subject]);
	} else {
		$previewsubject = "RE: ".stripinput($subject);
	}
	if ($message == "") {
		$previewmessage = LAN_221;
	} else {
		$previewmessage = stripinput($message);
		$previewmessage = parsesmileys($previewmessage, ENT_NOQUOTES);
	...
	if (isset($_POST['deletethread'])) {
		...
		if (dbrows($result) != 0) {
			while ($attach = dbarray($result)) {
				unlink(fusion_basedir."attachments/".$attach[attach_name]."[".$attach[attach_id]."]".$attach[attach_ext]);
			}
		}
...
<a href=\"index.php\">".LAN_248."</a><br><br>
		...
		if ($subject != "" && $message != "") {
			...
			$result = dbquery("UPDATE ".$fusion_prefix."posts SET post_subject='$subject', post_message='$message', post_edituser='$userdata[user_id].$userdata[user_name]', post_edittime='".time()."' WHERE post_id='$post_id'");
		} else {
			$error = LAN_241;
		}
		opentable(LAN_209);
...
<a href=\"index.php\">".LAN_248."</a><br><br>
	...
	if (!isset($_POST['previewchanges'])) {
		$subject = stripslashes($data[post_subject]);
...
<td class=\"forum1\"><input type=\"checkbox\" name=\"delete\" value=\"deletepost\"$delcheck>".LAN_282;
	if ($userdata[user_mod] > "0") {
		echo "<br>\n<input type=\"checkbox\" name=\"deletethread\" value=\"deletethread\"$delthreaadcheck>".LAN_283."<br>\n";
		if ($locked != "y") {
			echo "<input type=\"checkbox\" name=\"lock\" value=\"y\"$lockcheck>".LAN_284."\n";
		} else {
			echo "<input type=\"checkbox\" name=\"unlock\" value=\"y\"$unlockcheck>".LAN_285."\n";
		}
	}
	echo "</td>
</tr>
...
<input type=\"submit\" name=\"previewchanges\" value=\"".LAN_205."\" class=\"button\" style=\"width: 100px;\">
<input type=\"submit\" name=\"savechanges\" value=\"".LAN_209."\" class=\"button\" style=\"width: 100px;\">
</td>
</tr>
</form>
</table>\n";
