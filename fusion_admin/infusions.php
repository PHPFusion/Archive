...
if (isset($install)) {
	...
	if (dbrows($sql) != 0) {
		...
		if ($inf_link_name) {
			...
			$sql = dbquery("INSERT INTO ".$fusion_prefix."site_links VALUES('', '$inf_link_name', '".str_replace("../","",FUSION_INFUSIONS).$inf_folder."/".$inf_link_url."', '0', '0', '$link_order')");
		}
		if ($inf_create_tables[0]) {
			$i=0;
			while ($inf_create_tables[$i]) {
...
if (isset($upgrade)) {
	...
	if (dbrows($sql) != 0) {
		...
		if ($inf_upgrade_alter_tables[0]) {
			$i=0;
			while ($inf_upgrade_alter_tables[$i]) {
...
<td class='$row_color'><b>".LAN_404."</b> $inf_author<br>
<b>".LAN_405."</b> <a href='$inf_email'>$inf_email</a> | <b>".LAN_406."</b> <a href='$inf_web'>$inf_web</a><br>
<b>".LAN_407."</b> $inf_description<br><br>\n";
		if (!$data['inf_installed']) {
			echo "<a href='$PHP_SELF?install=".$data['inf_id']."' onClick='return InstallThis();'>".LAN_408."</a>\n";
		} else {
			echo "<a href='$PHP_SELF?uninstall=".$data['inf_id']."' onClick='return UninstallThis();'>".LAN_409."</a>\n";
		}
		if ($inf_version > $data['inf_version'] && $data['inf_installed']) {
			echo "| <a href='$PHP_SELF?upgrade=".$data['inf_id']."' onClick='return UpgradeThis();'>".LAN_410."</a>\n";
		}
		echo "</td>\n</tr>\n";
		$i++;
	}
