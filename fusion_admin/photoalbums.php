...
include FUSION_LANGUAGES.FUSION_LAN."admin/admin_photoalbums.php";
...
if(isset($_POST['btn_cancel'])){
	...
	if(file_exists(FUSION_PHOTOS."a".$album_id.".jpg")) unlink(FUSION_PHOTOS."a".$album_id.".jpg");
	header("Location: ".FUSION_SELF."?action=editalbum&album_id=$album_id");
...
<a href='".FUSION_SELF."'>".LAN_429."</a><br><br>
<a href='".FUSION_BASE."index.php'>".LAN_430."</a><br><br></center>\n";
			closetable();
		}else{
	...
	if(dbrows($result)!=0){
		...
		if(is_uploaded_file($_FILES['album_pic_file']['tmp_name'])){
			if($_FILES['album_pic_file']['size']>=$settings['album_max_b']){
				...
				if(file_exists(FUSION_PHOTOS."a".$album_id.".jpg")){unlink(FUSION_PHOTOS."a".$album_id.".jpg");}
				move_uploaded_file($_FILES['album_pic_file']['tmp_name'], FUSION_PHOTOS.basename($_FILES['album_pic_file']['tmp_name']));
				chmod(FUSION_PHOTOS.basename($_FILES['album_pic_file']['tmp_name']),0644);
				createThumbnail(FUSION_PHOTOS.basename($_FILES['album_pic_file']['tmp_name']),FUSION_PHOTOS."a".$album_id.".jpg",$settings['album_image_w'],$settings['album_image_h']);
				unlink(FUSION_PHOTOS.basename($_FILES['album_pic_file']['tmp_name']));
			}
		}
	}
	if($error<>""){
...
<tr><td>".LAN_408.":</td><td><input type='textbox' name='album_order' value='$album_order' maxlength='2' class='textbox' style='width: 40px;'>	<span class='small alt'>(";
	...
	if(file_exists(FUSION_PHOTOS."a".$album_id.".jpg")){
		echo "<br><br><a class='small' href='".FUSION_SELF."?action=deletealbumpic&album_id=$album_id'>".LAN_412."</a></td><td><img src='".FUSION_PHOTOS."a".$album_id.".jpg' border='1' alt='".LAN_418."' width='".$settings[album_image_w]."' height='".$settings[album_image_h]."'>";
	}else{
...
<td>".LAN_409."</td>
			...
			while($data=dbarray($result)){
				...
				if($rows!=1){
					if($album_order==1){
						$up_down=" <a href='".FUSION_SELF."?action=mdown&album_id=$album_id'><img src='".FUSION_THEME."images/down.gif' border='0' /></a>";
					}elseif($album_order < $rows){
						$up_down=" <a href='".FUSION_SELF."?action=mup&album_id=$album_id'><img src='".FUSION_THEME."images/up.gif' border='0' /></a>\n";
						$up_down.=" <a href='".FUSION_SELF."?action=mdown&album_id=$album_id'><img src='".FUSION_THEME."images/down.gif' border='0' /></a>";
					}else{
						$up_down=" <a href='".FUSION_SELF."?action=mup&album_id=$album_id'><img src='".FUSION_THEME."images/up.gif' border='0' /></a>";
					}
				...
				echo "<td>$album_id</td>
<td><a title='$album_info' href='".FUSION_SELF."?action=editalbum&album_id=".$data['album_id']."'>$album_title</a></td>
<td align='center'>";
				if(file_exists(FUSION_PHOTOS."a".$album_id.".jpg")){
...
<td>";
				...
				if ($photo_count==0){
					echo " | <a href='".FUSION_SELF."?action=deletealbum&album_id=".$data['album_id']."'>".LAN_412."</a>";
				}
				echo "</td></tr>";
				$r++;
			}
