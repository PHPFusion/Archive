...
".makePageNav($rowstart,$itemsperpage,$rows,3,"$PHP_SELF?forum_id=$forum_id&forum_cat=$forum_cat&thread_id=$thread_id&")."
</div>\n";
...
<td class='tbl1'>".LAN_505.strftime($settings['forumdate'], $data['post_datestamp']+($settings['timeoffset']*3600))."</td>
...
<a href='".fusion_basedir."fusion_public/attachments/".$attach['attach_name']."[".$attach['attach_id']."]".$attach['attach_ext']."'>".$attach['attach_name']."[".$attach['attach_id']."]".$attach['attach_ext']."</a>";
			}
		}
		if ($data['post_edittime'] != "0") {
			echo "<br>
...
<td class='tbl1'>\n";
		...
		if ($data['user_msn']) {
			echo "<a href='mailto:$data[user_msn]'><img src='".fusion_themedir."forum/msn.gif' alt='".$data['user_msn']."' border='0'></a> ";
		}
		if ($data['user_yahoo']) {
