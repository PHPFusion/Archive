...
global $settings,$locale,$userdata; $settings['rating_system']=1;
	...
	if ($settings['rating_system']=="1") {
		...
		if (iMEMBER) $d_rating=dbarray(dbquery("SELECT rating_vote,rating_datestamp FROM ".DB_PREFIX."ratings WHERE rating_item_id='".$rating_item_id."' AND rating_type='".$rating_type."' AND rating_user='".$userdata['user_id']."'"));
		if (!iMEMBER) {
