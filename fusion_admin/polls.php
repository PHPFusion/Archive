...
<input type='submit' name='delete' value='".LAN_422."' class='button'>
	...
	$i = 0; $opt = 1; $opt_count = ($opt_count ? $opt_count : 2);
	opentable((isset($poll_id) ? LAN_431 : LAN_430));
	echo "<form name='pollform' method='post' action='".$PHP_SELF.(isset($poll_id) ? "?poll_id=$poll_id" : "")."'>
...
<tr>\n";
	while ($i != $opt_count) {
		echo "<tr>
<td width='80'>".LAN_434."$opt</td><td><input type='text' name='poll_option[$i]' value='$poll_option[$i]' class='textbox' style='width:200px'></td>
</tr>\n";
		$i++; $opt++;
	}
	echo "</table>
...
<td align='center'><br>\n";
	...
	if (!isset($poll_id) || isset($poll_id) && $poll_ended == 0) {
		echo "<input type='hidden' name='opt_count' value='$count'>
