...
".LAN_430."<br><br>
	...
	if ($rows != 0) {
		...
		while ($data = dbarray($result)) {
			...
			echo "<a href='$PHP_SELF?cat_id=$cat_id&download_id=".$data['download_id']."' target='_blank'>".$data['download_title']."</a> - ".$data['download_filesize']." $new<br>\n";
			if ($data['download_description'] != "") {
				echo $data['download_description']."<br>\n";
			}
