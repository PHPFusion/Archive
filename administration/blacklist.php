...
if ($step == "delete") {
	...
	if (isset($_POST['blacklist_user'])) {
		...
		if ($blacklist_ip != "" && $blacklist_email == "") { $check = "blacklist_ip='$blacklist_ip'"; }
		elseif ($blacklist_ip != "" && $blacklist_email != "") { $check = "blacklist_ip='$blacklist_ip' OR blacklist_email='$blacklist_email'"; }
		elseif ($blacklist_ip == "" && $blacklist_email != "") {$check = "blacklist_email='$blacklist_email'"; }
		if (!dbcount("(*)", "blacklist", $check)) {
			if ($step == "edit") {
...
<td align='center' width='1%' class='$row_color' style='white-space:nowrap'><a href='".FUSION_SELF."?step=edit&amp;blacklist_id=".$data['blacklist_id']."'>".$locale['463']."</a> -
