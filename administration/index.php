...
if ($rows != 0) {
	...
	while ($data = dbarray($result)) {
		if (checkrights($data['admin_rights']) && $data['admin_link'] != "reserved") {
			...
			if ($admin_images) {
				echo "<a href='".$data['admin_link']."'><img src='".ADMIN."images/".$data['admin_image']."' alt='".$data['admin_title']."' style='border:0px;'><br>\n".$data['admin_title']."</a>";
			} else {
				echo "<img src='".THEME."images/bullet.gif' alt=''> <a href='".$data['admin_link']."'>".$data['admin_title']."</a>";
			}
			echo "</td>\n";
			$counter++;
		}
...
<td valign='top' width='33%' class='tbl1'>
