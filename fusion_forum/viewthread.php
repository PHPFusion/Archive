...
if (iMEMBER && $can_post) {
	...
	echo "<a href='post.php?action=newthread&forum_id=$forum_id'><img src='".FUSION_THEME."forum/newthread.gif' border='0'></a></td>\n";
}
echo "</tr>
</table>\n";
...
".makePageNav($rowstart,20,$rows,3,FUSION_SELF."?forum_id=$forum_id&thread_id=$thread_id&")."
</div>\n";
...
if ($rows != 0) {
	...
	while ($data = dbarray($result)) {
		...
		$is_mod = in_array($data['user_id'], $forum_mods) && $data['user_level'] < 251 ? true : false;
		echo "<tr>
...
<span class='alt'>".($is_mod ? USERF1 : getuserlevel($data['user_level']))."</span><br><br>\n";
		if ($data['user_avatar'] != "") {
