...
if (isset($_POST['save'])) {
	...
	$comments = isset($_POST['news_comments']) ? "1" : "0";
	$ratings = isset($_POST['news_ratings']) ? "1" : "0";
	if (isset($news_id)) {
		$result = dbquery("UPDATE ".$db_prefix."news SET news_subject='$subject', news_news='$body', news_extended='$body2', news_breaks='$breaks',".($start_date != 0 ? " news_datestamp='$start_date'," : "")." news_start='$start_date', news_end='$end_date', news_visibility='$news_visibility', news_allow_comments='$comments', news_allow_ratings='$ratings' WHERE news_id='$news_id'");
		opentable($locale['400']);
		echo "<center><br>
".$locale['401']."<br><br>
<a href='news.php'>".$locale['402']."</a><br><br>
<a href='index.php'>".$locale['403']."</a><br><br>
</center>\n";
		closetable();
	} else {
		$result = dbquery("INSERT INTO ".$db_prefix."news VALUES('', '$subject', '$body', '$body2', '$breaks', '".$userdata['user_id']."', '".($start_date != 0 ? $start_date : time())."', '$start_date', '$end_date', '$news_visibility', '0', '$comments', '$ratings')");
...
<a href='index.php'>".$locale['403']."</a><br><br>
	...
	if (isset($_POST['preview'])) {
		...
		$comments = isset($_POST['news_comments']) ? " checked" : "";
		$ratings = isset($_POST['news_ratings']) ? " checked" : "";
		opentable($subject);
		echo "$bodypreview\n";
		closetable();
...
<input type='submit' name='delete' value='".$locale['410']."' onclick='return DeleteNews()' class='button'>
	...
	if (isset($_POST['edit'])) {
		...
		if (dbrows($result) != 0) {
			...
			$breaks = ($data['news_breaks'] == "y" ? " checked" : "");
			$comments = ($data['news_allow_comments'] == "1" ? " checked" : "");
			$ratings = ($data['news_allow_ratings'] == "1" ? " checked" : "");
...
<td width='80%' class='tbl'><input type='text' name='subject' value='$subject' class='textbox' style='width: 250px'></td>
</tr>
<tr>
...
<input type='button' value='b' class='button' style='font-weight:boldwidth:25px' onClick=\"addText('body', '<b>', '</b>');\">
<input type='button' value='i' class='button' style='font-style:italicwidth:25px' onClick=\"addText('body', '<i>', '</i>');\">
<input type='button' value='u' class='button' style='text-decoration:underlinewidth:25px' onClick=\"addText('body', '<u>', '</u>');\">
<input type='button' value='link' class='button' style='width:35px' onClick=\"addText('body', '<a href=\'\' target=\'_blank\'>', '</a>');\">
<input type='button' value='img' class='button' style='width:35px' onClick=\"insertText('body', '<img src=\'".str_replace("../","",IMAGES_N)."\' style=\'margin:5px\' align=\'left\'>');\">
...
<tr>
<td valign='top' width='100' class='tbl'>".$locale['413']."</td><td class='tbl'><textarea name='body2' cols='95' rows='10' class='textbox'>$body2</textarea></td>
</tr>
<tr>
<td class='tbl'></td><td class='tbl'>
<input type='button' value='b' class='button' style='font-weight:boldwidth:25px' onClick=\"addText('body2', '<b>', '</b>');\">
<input type='button' value='i' class='button' style='font-style:italicwidth:25px' onClick=\"addText('body2', '<i>', '</i>');\">
<input type='button' value='u' class='button' style='text-decoration:underlinewidth:25px' onClick=\"addText('body2', '<u>', '</u>');\">
...
<input type='checkbox' name='news_comments' value='yes' onClick='SetRatings();'$comments> ".$locale['423']."<br>
<input type='checkbox' name='news_ratings' value='yes'$ratings> ".$locale['424']."<br>
</td>
</tr>
<tr>
