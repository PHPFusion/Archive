		...
		if (isset($_POST[save_cat])) {
			if ($action == "edit" && $t == "cat") {
				...
				if($cat_order<$data[forum_order]){
					$result = dbquery("UPDATE ".$fusion_prefix."forums SET forum_order=forum_order+1 WHERE forum_order>='$cat_order' AND forum_order<'$data[forum_order]'");
				} else if ($cat_order>$data[forum_order]){
					$result = dbquery("UPDATE ".$fusion_prefix."forums SET forum_order=forum_order-1 WHERE forum_order>'$data[forum_order]' AND forum_order<='$cat_order'");
				}				
				...
				$cat_name = stripinput($cat_name);
				$result = dbquery("UPDATE ".$fusion_prefix."forums SET forum_order=forum_order+1 WHERE forum_order>='$cat_order'");	
				$result = dbquery("INSERT INTO ".$fusion_prefix."forums VALUES('', '0', '$cat_name', '$cat_order', '', '0', '0', '0', '0', '0')");
				unset($action, $t, $cat_name, $cat_order);
			}
		}
		if (isset($_POST[save_forum])) {
			if ($action == "edit" && $t == "forum") {
				...
				if($forum_order<$data[forum_order]){
					$result = dbquery("UPDATE ".$fusion_prefix."forums SET forum_order=forum_order+1 WHERE forum_order>='$forum_order' AND forum_order<'$data[forum_order]'");
				} else if ($forum_order>$data[forum_order]){
					$result = dbquery("UPDATE ".$fusion_prefix."forums SET forum_order=forum_order-1 WHERE forum_order>'$data[forum_order]' AND forum_order<='$forum_order'");
				}				
				$forum_name = stripinput($forum_name);
				$forum_description = stripinput($forum_description);
				...
				unset($action, $t, $forum_cat, $forum_name, $forum_order, $forum_description, $forum_access);
			} else {
				$forum_name = stripinput($forum_name);
				$forum_description = stripinput($forum_description);
				$result = dbquery("UPDATE ".$fusion_prefix."forums SET forum_order=forum_order+1 WHERE forum_order>='$forum_order'");	
				$result = dbquery("INSERT INTO ".$fusion_prefix."forums VALUES('', '$forum_cat', '$forum_name', '$forum_order', '$forum_description', '$forum_access', '0', '0', '0', '0')");
				unset($action, $t, $forum_cat, $forum_name, $forum_order, $forum_description, $forum_access);
			}
		}
		if ($action == "edit") {
...
<table align=\"center\" width=\"300\" cellpadding=\"0\" cellspacing=\"0\" class=\"body\">
<tr>
<td>".LAN_440."<br>
...
<td class=\"altbg\"><a href=\"$PHP_SELF?action=edit&forum_id=$data[forum_id]&t=cat\">".LAN_481."</a> -
