...
if (isset($activate)) {
	...
	if ($settings['display_validation'] == "1") {
		...
		$result = dbquery("SELECT * FROM ".$db_prefix."temp WHERE temp_num='$temp_num'");
		if (dbrows($result) == 0) {
			$error .= $locale['410']."<br>\n";
		} else {
			if ($user_code != substr($temp_num, 17, 5)) {
				$error .= $locale['410']."<br>\n";
			} else {
				$result = dbquery("DELETE FROM ".$db_prefix."temp WHERE temp_num='$temp_num'");
			}
		}
	}
	...
	if ($error == "") {
		if ($settings['email_verification'] == "1") {
			...
			if (sendemail($username,$email,$settings['siteusername'],$settings['siteemail'],"Welcome to ".$settings['sitename'], $locale['450'].$activation_url)) {
				...
				if ($settings['display_validation'] == "1") $result = dbquery("DELETE FROM ".$db_prefix."temp WHERE temp_num='$temp_num'");
	...
	if ($settings['display_validation'] == "1") {
		...
		$rand_num = substr($temp_num, 17, 5); 
		$result = dbquery("INSERT INTO ".$db_prefix."temp VALUES('".time()."', '$temp_num')");
	}
	opentable($locale['400']);
...
<input type='radio' name='user_hide_email' value='0' checked>".$locale['u008']."</td>
	...
	if ($settings['display_validation'] == "1") {
		...
		if ($settings['validation_method'] == "image") {
			echo "<img src='?vimage=$rand_num'>\n";
		} else {
			echo "<b>$rand_num</b>\n";
		}
		echo "<input type='hidden' name='temp_num' value='$temp_num'>\n";
		unset($rand_num);
		echo "</td>\n</tr>\n";
		echo "<tr>
