...
if (isset($_POST['previewpost'])) {
	...
	if ($subject == "") {
		$previewsubject = LAN_220;
	} else {
		$previewsubject = stripinput($subject);
	}
	if ($message == "") {
		$previewmessage = LAN_221;
	} else {
		$previewmessage = stripinput($message);
...
if (isset($_POST['postnewthread'])) {
	...
	if ($subject != "" && $message != "") {
		$result = dbquery("UPDATE ".$fusion_prefix."forums SET forum_threads=forum_threads+1, forum_lastpost='".time()."', forum_lastuser='$userdata[user_id].$userdata[user_name]' WHERE forum_id='$forum_id'");
		$result = dbquery("INSERT INTO ".$fusion_prefix."threads VALUES('$forum_id', '', '$subject', '$userdata[user_id].$userdata[user_name]', '0', '0', '".time()."', '$userdata[user_id].$userdata[user_name]', '$makesticky', 'n')");
		$thread_id = mysql_insert_id();
		$result = dbquery("INSERT INTO ".$fusion_prefix."posts VALUES('$forum_id', '$thread_id', '', '$subject', '$message', '$sig', '$userdata[user_id].$userdata[user_name]', '".time()."', '', '0')");
	...
	if ($error) {
		...
		echo "<br>".LAN_242."<br><br>\n";
	}
	echo "<a href=\"viewforum.php?forum_id=$forum_id&forum_cat=$forum_up\">".LAN_247."</a><br><br>
<a href=\"index.php\">".LAN_248."</a><br><br></div>\n";
	closetable();
} else {
	if (!isset($_POST['previewpost'])) { $sigchecked = "checked"; }
	opentable(LAN_201);
	echo "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" width=\"100%\" class=\"forum-border\">
<form name=\"editform\" method=\"post\" action=\"$PHP_SELF?action=newthread&forum_id=$forum_id&forum_cat=$forum_cat\" enctype=\"multipart/form-data\">
<tr>
<td>
...
if ($userdata[user_mod] > "0") {
	echo "<input type=\"checkbox\" name=\"sticky\" value=\"makesticky\" $stickycheck>".LAN_280."</br>\n";
}
echo "<input type=\"checkbox\" name=\"showsig\" value=\"sig\"$sigchecked>".LAN_281."</td>
</tr>\n";
...
<input type=\"submit\" name=\"previewpost\" value=\"".LAN_200."\" class=\"button\" style=\"width: 100px;\">
<input type=\"submit\" name=\"postnewthread\" value=\"".LAN_201."\" class=\"button\" style=\"width: 100px;\">
</td>
</tr>
</form>
</table>\n";
