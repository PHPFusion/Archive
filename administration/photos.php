...
if ($action == "deletepic") {
	...
	$data = dbarray(dbquery("SELECT photo_id FROM ".$db_prefix."photos WHERE photo_order='$order'"));
	...
	if (!$error) {
		if ($action == "edit") {
			...
			$result = dbquery("UPDATE ".$db_prefix."photos SET photo_title='$photo_title', photo_description='$photo_description', ".$update_photos."photo_datestamp='".time()."', photo_allow_comments='$photo_comments', photo_allow_ratings='$photo_ratings' WHERE photo_id='$photo_id'");
...
<input type='submit' name='save_photo' value='".$locale['447']."' class='button'>\n";
	if ($action) {
		...
		echo "<input type='hidden' name='photo_order' value='$photo_order'>\n";
	}
	echo "</td></tr>\n</table></form>\n";
	closetable();
}
