...
if (iMEMBER) {
	...
	if (!$step) {
		...
		if (dbrows($result) != 0) {
			$i = 0;
			echo "<form name='message_form' method='post' action='$PHP_SELF'>
...
<td width='110'>".LAN_403."</td>
			...
			while ($data = dbarray($result)) {
				...
				echo "<tr class='$row_color'>\n<td><input type='checkbox' name='del_message[$i]' value='".$data['message_id']."'></td>\n";
				echo "<td><a href='messages.php?step=read&message_id=".$data['message_id']."'>";
...
<td>".showdate("shortdate", $data['message_datestamp'])."</td>
</tr>\n";
				$i++;
			}
			echo "</table>
...
function DeleteAll() {
	return confirm('".LAN_412."');
		...
		if (!$message_id || !isNum($message_id)) { header("Location:index.php"); exit; }
		$result = dbquery(
...
<a href='messages.php'>".LAN_521."</a></td>
		...
		if (isset($_POST['send_message'])) {
			...
			if (!$error) {
				...
				$smileys = $_POST['disable_smileys'] ? "n" : "y";
...
<a href='index.php'>".LAN_451."</a></div></td>
	...
	<a href='$PHP_SELF'>".LAN_455."</a>.<br><br>
			...
			opentable(LAN_430);
			echo "<form name='inputform' method='post' action='$PHP_SELF?step=send&user_id=$user_id'>
