...
if ($msg_ids && $check_count > 0) {
	if (isset($_POST['save_msg'])) {
		$archive_total = dbcount("(message_id)", "messages", "message_to='".$userdata['user_id']."' AND message_folder='2'");
		if (($archive_total + $check_count) <= $msg_settings['pm_savebox']) {
		...
		$inbox_total = dbcount("(message_id)", "messages", "message_to='".$userdata['user_id']."' AND message_folder='0'");
		if (($inbox_total + $check_count) <= $msg_settings['pm_inbox']) {
...
if (isset($_POST['send_message'])) {
	...
	if (dbrows($result)) {
		...
		$my_settings['pm_save_sent'] = 0; $my_settings['pm_email_notify'] = 0;
	}
	...
	if (isset($_POST['chk_sendtoall']) && isNum($_POST['msg_to_group'])) {
		...
		if ($msg_to_group == "101" || $msg_to_group == "102" || $msg_to_group == "103") {
			...
			while ($data = dbarray($result)) {
				...
				if ($data['user_id'] != $userdata['user_id']) {
					...
					if ($data['pm_email_notify'] == "1") sendemail($data['user_name'],$data['user_email'],$settings['siteusername'],$settings['siteemail'],$locale['625'],$udata['user_name'].$locale['626']);
				}
			}
		} else {
			$result = dbquery(
		...
		$data = dbarray($result);
		if ($data['user_id'] != $userdata['user_id']) {
			if (($data['message_count'] + 1) <= $msg_settings['pm_inbox']) {
	...
	if (!$error) {
		$result =dbquery(
			"SELECT COUNT(message_id) AS outbox_count, MAX(message_id) AS last_message FROM ".$db_prefix."messages
			WHERE message_to='".$userdata['user_id']."' AND message_folder='1' GROUP BY message_to"
		);
		$cdata = dbarray($result);
		if ($my_settings['pm_save_sent']) {
			if (($cdata['outbox_count'] + 1) > $message_settings['pm_savebox']) {
				$result = dbquery("DELETE FROM ".$db_prefix."messages WHERE message_id='".$cdata['message_id']."' AND message_to='".$userdata['user_id']."'");
			}
			if (isset($_POST['chk_sendtoall']) && isNum($_POST['msg_to_group'])) {
...
if (!isset($msg_send) && !isset($msg_read) && $folder != "options") {
	...
	$bdata['inbox_total'] = $bdata['inbox_total'] ? $bdata['inbox_total'] : "0";
	$bdata['outbox_total'] = $bdata['outbox_total'] ? $bdata['outbox_total'] : "0";
	$bdata['archive_total'] = $bdata['archive_total'] ? $bdata['archive_total'] : "0";
	if ($folder == "inbox") {
	...
	echo "<form name='pm_form' method='post' action='".FUSION_SELF."?folder=$folder'>\n";
	echo "<table cellpadding='0' cellspacing='0' width='100%'>\n";
	echo "<tr>\n<td align='left' width='100%' class='tbl'><input type='button' class='button' title='".$locale['500']."' onclick=\"location.href='".FUSION_SELF."?msg_send=0'\" value='".$locale['401']."'></td>\n";
	echo "<td width='25%' class='tbl2' style='white-space:nowrap;font-weight:".($folder=="inbox"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=inbox'>".$locale['402']." [".$bdata['inbox_total']."/".$msg_settings['pm_inbox']."]</a></td>\n";
	echo "<td width='25%' class='tbl2' style='white-space:nowrap;font-weight:".($folder=="outbox"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=outbox'>".$locale['403']." [".$bdata['outbox_total']."/".$msg_settings['pm_sentbox']."]</a></td>\n";
	echo "<td width='25%' class='tbl2' style='white-space:nowrap;font-weight:".($folder=="archive"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=archive'>".$locale['404']." [".$bdata['archive_total']."/".$msg_settings['pm_savebox']."]</a></td>\n";
	echo "<td width='25%' class='tbl2' style='white-space:nowrap;font-weight:".($folder=="options"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=options'>".$locale['425']."</a></td>\n";
	echo "</tr>\n</table>\n";
	if ($total_rows) {
		...
		echo "<td width='25%' class='tbl2' style='white-space:nowrap'>".$locale['407']."</td>\n</tr>\n";
		while ($data = dbarray($result)) {
		...
		echo "<tr>\n<td class='tbl'><input type='button' class='button' onClick=\"javascript:setChecked('pm_form','check_mark[]',1);return false;\" value='".$locale['410']."'> |
		<input type='button' class='button' onClick=\"javascript:setChecked('pm_form','check_mark[]',0);return false;\" value='".$locale['411']."'</td>\n";
	...
	echo "<table cellpadding='0' cellspacing='0' width='100%'>\n";
	echo "<tr>\n<td align='left' width='100%' class='tbl'><input type='button' class='button' title='".$locale['500']."' onclick=\"location.href='".FUSION_SELF."?msg_send=0'\" value='".$locale['401']."'></td>\n";
	echo "<td width='25%' class='tbl' style='white-space:nowrap;font-weight:".($folder=="inbox"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=inbox'>".$locale['402']." [".$bdata['inbox_total']."/".$msg_settings['pm_inbox']."]</a></td>\n";
	echo "<td width='25%' class='tbl' style='white-space:nowrap;font-weight:".($folder=="outbox"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=outbox'>".$locale['403']." [".$bdata['outbox_total']."/".$msg_settings['pm_sentbox']."]</a></td>\n";
	...
	echo "</table>\n";
	echo "<table border='0' cellpadding='0' cellspacing='0' width='100%'>\n";
	echo "<tr>\n<td colspan='2' class='tbl'><input type='button' class='button' value='".$locale['432']."' onclick=\"location.href='".FUSION_SELF."?folder=$folder'\"></td>\n";
