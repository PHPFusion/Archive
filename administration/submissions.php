...
if ($stage == "2" && $t == "n") {
	if (isset($_POST['publish'])) {
		...
		$result = dbquery("INSERT INTO ".$db_prefix."news VALUES('', '$news_subject', '$news_cat', '$news_body', '', '$news_breaks', '".$data['user_id']."', '".time()."', '0', '0', '0', '0', '1', '1')");
		$result = dbquery("DELETE FROM ".$db_prefix."submissions WHERE submit_id='$submit_id'");
...
if ($stage == "2" && $t == "a") {
	if (isset($_POST['publish'])) {
		...
		$result = dbquery("INSERT INTO ".$db_prefix."articles VALUES('', '$article_cat', '$article_subject', '$article_snippet', '$article_body', '$article_breaks', '".$data['user_id']."', '".time()."', '0', '1', '1')");
		$result = dbquery("DELETE FROM ".$db_prefix."submissions WHERE submit_id='$submit_id'");
...
<a href='index.php'>".$locale['403']."</a></div><br>\n";
		...
		$article_snippet = stripslashes($submit_criteria['article_snippet']);
		$article_body = stripslashes($submit_criteria['article_body']);
