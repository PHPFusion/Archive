...
if (isset($_POST['btn_do_restore'])) {
	...
	$result = gzfile(ADMIN."/db_backups/temp.txt");
	...
	if (is_uploaded_file($_FILES['upload_backup_file']['tmp_name'])) {
		...
		move_uploaded_file($_FILES['upload_backup_file']['tmp_name'], ADMIN."/db_backups/temp.txt");
		$backup_data = gzfile(ADMIN."/db_backups/temp.txt");
	} else {
		fallback(FUSION_SELF.$aidlink);
	}
	$info_tbls=array();
