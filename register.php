...
if (isset($activate)) {
	...
	if (dbrows($result) != 0) {
		...
		$result = dbquery("INSERT INTO ".$fusion_prefix."users VALUES('', '".$user_info['user_name']."', '".md5($user_info['user_password'])."', '".$user_info['user_email']."', '".$user_info['user_hide_email']."', '', '0000-00-00', '', '', '', '', 'Default', '0', '', '', '0', '".time()."', '0', '1', '0')");
...
".LAN_453."<br><br>
	...
	if ($error == "") {
		if ($settings['email_verification'] == "1") {
			...
			if (sendemail($username, $_POST['email'], "Welcome to ".$settings['sitename'], LAN_450.$activation_url)) {
				$user_info = serialize(array(
					"user_name" => $username,
					"user_password" => $_POST['password1'],
					"user_email" => $_POST['email'],
					"user_hide_email" => $_POST['email']
				));
			...
			$result = dbquery("INSERT INTO ".$fusion_prefix."users VALUES('', '$username', md5('$password1'), '$email', '$hide_email', '$user_location', '$user_birthdate', '$user_icq', '$user_msn', '$user_yahoo', '$user_web', '".$_POST['user_theme']."', '".$_POST['user_offset']."', '', '$user_sig', '0', '".time()."', '0', '1', '0')");
			opentable(LAN_400);
			echo "<center><br>\n".LAN_451."<br><br>\n</center>\n";
			closetable();
		}
		...
		opentable(LAN_454);
		echo "<center><br>\n".LAN_456."<br><br>\n$error<br>\n<a href='$PHP_SELF'>".LAN_457."</a></div></br>\n";
		closetable();
	}
} else {
	if ($settings['email_verification'] == "0") {
