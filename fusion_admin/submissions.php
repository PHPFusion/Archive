...
<td align='right' width='100'><span class='small'><a href='$PHP_SELF?stage=2&t=l&submit_id=".$data['submit_id']."'>".LAN_417."</a></span> |
<span class='small'><a href='$PHP_SELF?delete=".$data['submit_id']."'>".LAN_418."</a></span></td></tr>";
			}
		} else {
...
<td align='right' width='100'><span class='small'><a href='$PHP_SELF?stage=2&t=n&submit_id=".$data['submit_id']."'>".LAN_417."</a></span> |
<span class='small'><a href='$PHP_SELF?delete=".$data['submit_id']."'>".LAN_418."</a></span></td></tr>";
			}
		} else {
...
<td align='right' width='100'><span class='small'><a href='$PHP_SELF?stage=2&t=a&submit_id=".$data['submit_id']."'>".LAN_417."</a></span> |
<span class='small'><a href='$PHP_SELF?delete=".$data['submit_id']."'>".LAN_418."</a></span></td></tr>";
			}
		} else {
...
<a href='index.php'>".LAN_403."</a></div><br>\n";
		...
		opentable(LAN_440);
		echo "<form name='publish' method='post' action='$PHP_SELF?stage=2&t=l&submit_id=$submit_id'>
...
if ($stage == "2" && $t == "n") {
	if (isset($_POST['publish'])) {
		...
		$submit_criteria = unserialize($data['submit_criteria']);
		$news_body = addslashes($submit_criteria['news_body']);
		$result = dbquery("INSERT INTO ".$fusion_prefix."news VALUES('', '".$submit_criteria['news_subject']."', '$news_body', '', '".$submit_criteria['news_breaks']."', '".$data['user_id']."', '".time()."', '0', '0', '0')");
		$result = dbquery("DELETE FROM ".$fusion_prefix."submissions WHERE submit_id='$submit_id'");
		opentable(LAN_490);
...
<a href='index.php'>".LAN_403."</a></div><br>\n";
		...
		$news = stripslashes($submit_criteria['news_body']);
		opentable($subject);
		echo ($submit_criteria['news_breaks'] == "y" ? nl2br($news) : $news);
		closetable();
		tablebreak();
		opentable(LAN_500);
		echo "<form name='publish' method='post' action='$PHP_SELF?sub=submissions&stage=2&t=n&submit_id=$submit_id'>
<center>
...
if ($stage == "2" && $t == "a") {
	if (isset($_POST['publish'])) {
		...
		$submit_criteria = unserialize($data['submit_criteria']);
		$description = addslashes($submit_criteria['article_description']);
		$article = addslashes($submit_criteria['article_body']);
...
<a href='index.php'>".LAN_403."</a></div><br>\n";
		...
		$description = stripslashes($submit_criteria['article_description']);
		$article = stripslashes($submit_criteria['article_body']);
		opentable($subject);
		echo $description;
		closetable();
		tablebreak();
		...
		tablebreak();
		opentable(LAN_540);
		echo "<form name='publish' method='post' action='$PHP_SELF?sub=submissions&stage=2&t=a&submit_id=$submit_id'>
<center>
