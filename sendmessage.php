		...
		if (isset($reply_id)) {
			...
			if ($data2[message_subject] != "") {
				if (!strstr($data2[message_subject], "RE: ")) {
					$subject = "RE: ".stripslashes($data2[message_subject]);
				} else {
					$subject = stripslashes($data2[message_subject]);
				}
			}
		} else {
...
<input type=\"submit\" name=\"send_message\" value=\"".LAN_273."\" class=\"button\" style=\"width: 100px;\"></div>
</td>
</tr>
</form>
</table>\n";
