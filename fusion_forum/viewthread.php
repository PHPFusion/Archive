...
if ($access_level == 0) {
	...
	if ($userdata[user_mod] >= $access_level) { $access = "yes"; } else { $access = "no"; }
}
...
<span class=\"alt\">".getmodlevel($data2[user_mod])."</span><br><br>\n";
		if ($data2[user_avatar] != "") {
			echo "<img src=\"".fusion_basedir."fusion_public/avatars/$data2[user_avatar]\"><br><br>\n";
			$height = "170";
		} else {
			$height = "55";
		}
...
<td align=\"right\" class=\"forum1\">\n";
		if ($userdata != "") {
			if ($locked != "y") {
				...
				echo "<a href=\"post.php?action=reply&forum_id=$forum_id&forum_cat=$forum_cat&thread_id=$data[thread_id]&post_id=$data[post_id]&quote=$data[post_id]\"><img src=\"images/quote.gif\" alt=\"quote\" border=\"0\"></a>\n";
			} else {
