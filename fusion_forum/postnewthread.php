...
if (isset($_POST['postnewthread'])) {
	...
	if (Member()) {
		if ($subject != "" && $message != "") {
			$result = dbquery("UPDATE ".$fusion_prefix."forums SET forum_threads=forum_threads+1, forum_posts=forum_posts+1, forum_lastpost='".time()."', forum_lastuser='$userdata[user_id]' WHERE forum_id='$forum_id'");
			$result = dbquery("INSERT INTO ".$fusion_prefix."threads VALUES('$forum_id', '', '$subject', '$userdata[user_id].$userdata[user_name]', '0', '0', '".time()."', '$userdata[user_id]', '$sticky', 'n')");
...
<a href=\"index.php\">".LAN_448."</a><br><br></div>\n";
	closetable();
} else {
	if (!isset($_POST['previewpost'])) { $sig_checked = " checked"; }
	opentable(LAN_401);
	echo "<form name=\"editform\" method=\"post\" action=\"$PHP_SELF?action=newthread&forum_id=$forum_id&forum_cat=$forum_cat\" enctype=\"multipart/form-data\">
...
<td class=\"forum2\"><input type=\"text\" name=\"subject\" value=\"$subject\" class=\"textbox\" maxlength=\"255\" style=\"width: 250px\"></td>
</tr>
<tr>
...
<td class=\"forum1\">\n";
	if (Moderator()) {
		echo "<input type=\"checkbox\" name=\"sticky\" value=\"makesticky\" $sticky_check>".LAN_480."<br>\n";
	}
	echo "<input type=\"checkbox\" name=\"disable_smileys\" value=\"y\"$disable_smileys_check>".LAN_483;
