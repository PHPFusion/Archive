...
if (dbrows($result) != 0) {
	...
	$result2 = dbquery("SELECT * FROM ".$fusion_prefix."poll_votes WHERE vote_user='".$userdata['user_id']."' AND poll_id='".$data['poll_id']."'");
	if (!dbrows($result2) && $data['poll_ended'] == 0) {
		...
		echo "<form name='voteform' method='post' action='".$PHP_SELF.(FUSION_QUERY ? "?".FUSION_QUERY : "")."'>
...
<a href='".FUSION_BASE."polls_archive.php' class='side'>".LAN_108."</a> <img src='".FUSION_THEME."images/bulletb.gif'>\n";
		}
		echo "</center>\n";
	}
} else {
